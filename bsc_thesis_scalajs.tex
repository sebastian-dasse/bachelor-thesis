...
Von modernen Weboberflächen wird Interaktivität in einem Maße erwartet, wie es noch vor wenigen Jahren ausgewachsenen Desktopanwendungen vorbehalten war. Klassische Webseiten sind echten Web-Anwendungen (\emph{Rich Internet Applications}) gewichen und die Zahl an Anwendungen, die als Online-Dienst in der "`Cloud"' angeboten werden (\emph{Software as a Service}), steigt.
...
Das führt dazu, das für solche Standardprobleme jedes Mal das "`Rad neu erfunden"' wird oder neu auszuhandeln ist, welcher Weg beschritten werden soll. Häufig bieten hier Frameworks oder Librarys von Drittanbietern Lösungen. Allerdings um den Preis zusätzlicher Lernzeit.
...
Hierfür sind verschiedene Fragen zu klären. Es gilt, die grundlegenden Mechanismen dieses Werkzeugs zu erproben und den Konfigurationsaufwand einzuschätzen. Dann soll Klarheit gewonnen werden, wie sich häufige Anforderungen aus dem Bereich der Web-Entwicklung mit Scala.js umsetzen lassen und ob der Entwickler hierbei an Grenzen stößt. Als wichtiger Punkt soll auch betrachtet werden, welche Auswirkungen die Verwendung von Scala.js auf die Code-Qualität\footnote{Dieser Begriff wird in Kapitel \ref{chap:methods} eingegrenzt.} hat. Aber auch der Komfort des Arbeitens mit Scala.js soll untersucht werden, etwa mit welcher Schnelligkeit entwickelt werden kann. Außerdem ist die Frage wichtig, ob die Leistungsfähigkeit von Scala.js-Anwendungen mit solchen, die auf herkömmliche Weise in JavaScript entwickelt wurden, mithalten kann.
...
Kapitel \ref{chap:scala.js} liefert einen Überblick über Scala.js. Dazu werden zunächst die konzeptionellen Grundlagen der Implementierung erklärt. Danach wird auf einige Abweichungen von der Scala-Semantik eingegangen und die Interoperabilität mit JavaScript erläutert. Schließlich wird das \TODOi{technische Ökosystem beschrieben, das Scala.js bietet}. \REDOi
...
Kapitel \ref{chap:case-studies} stellt  \TODOi{sieben (?)} abgegrenzte Fallstudien vor, in denen anhand praktisch umgesetzter Beispielimplementierungen wichtige Aspekte von Scala.js untersucht und eingeschätzt werden.
...
Der hauptsächliche Zweck von Scala.js besteht darin, bestimmten Mängeln der Sprache JavaScript dadurch zu begegnen, dass sie als eine Ziel-Plattform der Sprache Scala betrachtet wird. \REDOi Für ein besseres Verständnis sollen hier deshalb zunächst beide Sprachen, Scala und JavaScript, besprochen werden. Im Anschluss folgt zur \TODOi{besseren} Einordnung ein knapper Überblick über die Web-Kerntechnologien \ac{HTML} und \ac{CSS} und sowie eine kurze Vorstellung des Buildtools sbt.
...
\ac{HTML} wird vom \ac{W3C}\footnote{\url{http://www.w3.org/}} standardisiert. Die aktuelle fünfte Fassung (kurz: HTML5) wurde Ende 2014 als "`W3C Recommendation"'\footnote{\url{http://www.w3.org/TR/html5/}} veröffentlicht. Mit HTML5 wurden einige semantische Tags neu eingeführt (\html{section}, \html{article}, \html{nav}, \html{head}, \html{footer}, \html{aside}). Auch kamen zwei Tags für Multimediainhalte hinhzu: \html{<audio>} und \html{<video>}\footnote{Es existiert allerdings bis heute kein Standard dafür, welche Multimediaformate verlässlich von allen Browsern unterstützt werden.}.

Wenn von HTML5 die Rede ist, sind neben der Auszeichnungssprache \ac{HTML} aber auch zahlreiche zusätzliche JavaScript-APIs mit gemeint, wie: Canvas, WebGL, SVG, XMLHttpRequest, Drag and drop, Web Workers, WebSockets, File API, Web Storage API und viele andere\footnote{Eine Übersicht findet sich zum Beispiel hier: \url{https://developer.mozilla.org/en-US/docs/Web/Guide/HTML/HTML5}} Bis heute existiert hierzu allerdings keine offizielle, leicht navigierbare HTML5-API-Dokumentation\footnote{Eine inoffizielle \ac{API}-Dokumentation: \url{http://html5index.org/}}.

Eine besonders wichtige \ac{API} stellt das \ac{DOM} dar. Hierbei handelt es sich um eine Baumstruktur für Dokumente, mit Knoten für die einzelnen Elemente. Der Wurzelknoten ist das \html{document}-Objekt. JavaScript kann das \ac{DOM} manipulieren, durch das \ac{DOM} navigierenm, alle Elemente und deren Attribute ändern und Elemente erzeugen, hizufügen oder entfernen und auf Ereignisse der im \ac{DOM} repräsentierten Website reagieren. In der aktuellen Version (\ac{DOM} Level 3, von 2004) wird es von allen gängigen Browsern unterstützt.
...
\begin{figure}[!h]
    ...
    \caption{Der Scala.js-Kompilationsprozess. \cite[Folie 17]{doeraene2014.WHB}}
...
Zum Einstieg wird ein Tutorial wird ein Tutorial und beispielhafte Projektstrukturen, die als Grundlage für eigene Projekt (\emph{Skeletons}. 
    - E-Book: Hands-on Scala.js
Mit Scala.jsFiddle existiert ein Online-Scratchpad, mit dem sich 


... interessante Vortragsvideos der Scala.js-Entwickler verlinkt.

, Antworten auf häufig gestellte Fragen. 
Einträge bei Stackoverflow
Mailingliste, ein Scala.js-Chatroom
...
- \ac{API}-Docs
- Tutorial
- News Group / Forum  ->  %https://groups.google.com/forum/?fromgroups#!forum/scala-js
- Versionsgeschichte
- Hands-on Scala.js
- Scala-Js-Fiddle
...
\section{Verwendete Hard- und Software sowie Messverfahren}
...
\begin{figure}[!h]
	...
	\caption{Ohne Sourcemaps: Wenig aussagekräftiger Stacktrace \protect\subref{fig:sourcemaps1}, navigierbar zu schwer verständlichem, generierten JavaScript-Code \protect\subref{fig:sourcemaps2}.}
...
\begin{figure}[!h]
	...
	\caption{Unterstützung durch Source Maps: Diagnosestärkerer Stacktrace \protect\subref{fig:sourcemaps3}, navigierbar zu verständlichem Scala-Code \protect\subref{fig:sourcemaps4}.}
...
>> zu async --> url -> %http://docs.scala-lang.org/sips/pending/async.html
...
einfache GUI in klassischem \ac{HTML}/JavaScript, die Scala.js-generierte Statistik-Lib verwendet (siehe Abb. \ref{fig:stat-lib-gui}) - funktioniert sehr gut
...
- \TODOi{(siehe Abbildung 6.9(??) und Tabelle 6.4 (??))}
