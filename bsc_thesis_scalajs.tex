...
JavaScript ist eine dynamische Skriptsprache, die es erlaubt, objektorientiert und funktional zu programmieren. Die Sprache wurde 1995 von Brendan Eich für Netscape innerhalb von zehn Tagen entwickelt\cite{wikipedia.JS}. Der primäre Zweck von JavaScript ist es, aus statischen Webseiten interaktive Webanwendungen machen\cite[S. 361]{flanagan2011.JDG}.
...
Der Sprachstandard wird unter dem offiziellen Namen ECMAScript ((kurz: ES)) von der Organisation Ecma International standardisiert (als Standard ECMA-262). Der aktuell von allen wichtigen Browsern unterstützte Standard ECMAScript 5.1 stammt aus dem Jahr 2011.\footnote{
...
für andere vordefinierte JavaScript-Typen existieren Definitionen in scala.scalajs.js:
...
Das Paket \texttt{scala.scalajs.js} enthält mit \texttt{Math} sowie \texttt{JSNumberOps} und \texttt{JSStringOps} auch Fassaden, um auf native JavaScript-Methoden und Konstanten beziehungsweise Operationen für Zahlen Strings zurückzugreifen.
...
Scope (z.B. Test) festlegen durch nachgestelltes \texttt{\% "'test"'}
...
wird festgelegt, dass die Scala.js-Fassaden-Bibliothek im \texttt{FullOpt}-Modus die minifizierte Version von jQuery in die 
...
- statische Typen für jQuery. (Alternativ gibt es die Bibliothek \texttt{jquery-facade}, die aber nicht ausprobiert wurde.))
...
Die Einbindung von \texttt{scala-js-jquery} unkompliziert. Dau muss lediglich
\begin{lstlisting}[style=snippet]
libraryDependencies += "be.doeraene" %%% "scalajs-jquery" % "0.8.0"
\end{lstlisting}
in der \texttt{buil.sbt} hinzugefügt werden.

Etwas schwieriger war aber, zu verstehen wie sich hier und generell minifizierte Versionen für die im \texttt{target/}-Ordner erzeugten JavaScript-Dependencies erzwingen lassen (siehe (???)).

Für Tests wurde \texttt{uTest} verwendet und ensprechend den \texttt{libraryDependencies} hinzugefügt. Um dem DOM für Tests verwenden zu können, muss wie in Kapitel (sbt-Integration ???) beschrieben, der Eintrag
\begin{lstlisting}[style=snippet]
...
% % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % %
\section{Hello ScalaTags: HTML-Templating}
...
Betrachtet wurden vier Möglichkeiten, in Scala.js HTML zu generieren. Dazu sei in der \texttt{index.html} ein
\texttt{<div>} mit der ID "`container"' definiert. Ferner gehen wir davon aus, dass die Referenz darauf im Scala.js-Programm bekannt ist durch

\begin{lstlisting}[language=Scala, style=snippet]
val container: org.scalajs.dom.raw.Element = document.getElementById("container")
\end{lstlisting}

Mithilfe von Scala-js-dom können die DOM-Methoden \texttt{createElement()} zum erzeugen von HTML-Elementen und \texttt{appendChild()} zum einhängen von Elementen in den DOM-Tree verwendet werden.
...
\begin{lstlisting}[language=Scala, style=snippet]
\texttt{document.createElement("xyz")}   // erzeugt: <xyz>Hello world!?</xyz>
\texttt{document.createElement("\@\$#!")} // f"uhrt zu einem Laufzeitfehler
\end{lstlisting}

Eine weitere Möglichkeit, ist die direkte Manipulation der \texttt{innerHTML}-Property, der HTML-Code als String zugewiesen werden kann.
...
Mit ScalaTags nun ist es möglich, in Scala typsichere HTML-Templates zu schreiben, aus denen sich valider HTML-Code generieren lässt. Attribute wie etwa \texttt{id} und \texttt{class} können ebenso wie CSS-Styles gesetzt werden.\footnote{
	ScalaTags bietet bei Bedarf sogar eine DSL, um CSS-Stylesheets zu generieren.}
...
Für die Client-Seite existiert zusätzlich die bessere Möglichkeit, die Templates direkt in \texttt{dom.Element}s umzuwandeln:
...
Damit ist es möglich typsicher, deklarativ und nahe an HTML, gleichzeitig (schon durch das Wegfallen der schließenden Tags) sehr lesbaren Code zu schreiben. Hinzu kommt die Möglichkeit, Kontrollfluss-Anweisungen wie \texttt{for} und \texttt{if/else} einzustreuen (ein Beispiel hierfür: Listing \ref{code:setupCurrencySelect}).
...
\begin{lstlisting}[language=Scala, caption={Timer-Klasse des Canvas-Beispiels.}]
...
Dieser Methode muss eine Funktion \texttt{String => Unit}) übergeben werden, die den Auswahlwert entgegennimmt, so dass der Umrechner passend aktualisiert werden kann. Die Liste der möglichen \texttt{<option>}s lässt sich prägnant mithilfe einer \texttt{for}-Comprehension ausdrücken. Das Auswahllisten-Elemente, bei dem es sich um ein \texttt{scalatags.JsDom.TypedTag[html.Select]} handelt, muss durch Aufruf der Methode \texttt{render} in den Fassadentyp \texttt{html.Select} der Scala-js-dom-Bibliothek umgewandelt werden. Dadurch haben wir vollen, typsicheren Zugriff auf die Methoden und Attribute ((die in der DOM-API spezifiziert sind)). Ein \texttt{onchange}-Handler ließe sich eigentlich auch innerhalb des \texttt{select()}-Tags schreiben, allerdings ließe sich so nicht auf das \texttt{value}-Attribut zugreifen.

Nach dieser Vorbereitung kann das Umrechnungsformular sehr übersichtlich und deklarativ geschrieben werden:

\begin{lstlisting}[language=Scala, caption={Deklaration eines Formulars zur Währungsumrechnung mit ScalaTags.}]
...
Für die Umrechnung wurde eine Klasse \texttt{Converter} definiert. Als Quelle für die Umrechnungskurse sollte die Yahoo! Finance API\footnote{
	\url{https://code.google.com/p/yahoo-finance-managed/wiki/YQLAPI}}
genutzt werden, die auf HTTP-Anfrage hin Ergebnisse im JSON-Format\footnote{
	JavaScript Object Notation, ein kompaktes Format zum Datenaustausch.}
liefert. Anfrage werden in einer Query-Language formuliert. Die Anfrage:
...
Dank Scala-js-dom kann das \texttt{XMLHttpRequest}-Objekt\footnote{
	Eine ursprünglich von Microsoft entwickelte API zur Client-Server-Kommunikation, die von den großen Browserherstellern Mozilla, Apple, and Google unterstützt und gegenwärtig vom W3C standardisiert wird.\cite{mdn.XHR}}
ganz ähnlich wie mit reinem JavaScript verwendet werden (allerdings typsicher).
...
Der \texttt{onload}-Handler wird als anonyme Funktion übergeben, semantisch ganz analog zu JavaScript:

\begin{lstlisting}[language=JavaScript, style=snippet]
xhr.onload = function(e) { /* ... */ } // JavaScript
\end{lstlisting}

\texttt{XMLHttpRequest} verlangt relativ viel Handarbeit, bietet aber dafür einen sehr direkten Zugriff auf das HTTP-Protokoll. Für viele Anwendungsfälle ist das jedoch gar nicht nötig. Damit diese prägnanter geschrieben werden können, bietet Scala-js-dom Bequemlichkeitsmethoden für die gebräuchlichsten HTTP-Methoden (GET, POST, PUT und DELETE). Diese Methoden geben eine \texttt{Future[dom.XMLHttpRequest]}\footnote{
	Futures in Scala: \url{http://docs.scala-lang.org/overviews/core/futures.html}}
zurück. Damit Futures und Promises in Scala.js verwendet werden können, wird ein Ausführungskontext benötigt.. \cite[\#Extensions]{scala-js-dom.DOC} Zwei zusätzliche Importe sind nötig, der GET-Aufruf schrumpft dafür auf einen Einzeiler:
...
So definieren wir eine \texttt{Converter}-Klasse, die immutable ist. Für jedes Währungspaar soll ein neue Instanz erzeugt werden.
...
Durch den Aufruf von \texttt{JSON.parse} wird der Antworttext deserialisiert. Dabei erhalten wir ein dynamisch getyptes Objekt. Hier muss nun besonders sorgfältig vorgegangen werden, da der Compiler uns nicht vor Tippfehlern schützt. Die Rate muss zur sinnvollen Verwendung zuerst in einen String, und dieser dann in einen \texttt{Double}-Wert umgewandelt werden.
...
Zur Formatierung des Ausgabewerts \texttt{dstAmount: Double} muss \texttt{"'\%.4f"'.format(dstAmount)} oder \texttt{f"'\$dstAmount\%.4f"'} verwendet werden, denn die Java-Methode \texttt{String.format()} funktioniert nicht.

Es ist wäre hier auch möglich, durch den Import von \texttt{import scala.scalajs.js.JSNumberOps.\_}
auf native JavaScript-Methoden zurückzugreifen und \texttt{dstAmount.toFixed(4)} zu schreiben.
