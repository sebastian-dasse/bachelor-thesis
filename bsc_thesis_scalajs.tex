...
\TODO{>> andere Sprachen die nach JavaScript kompilieren}
\TODO{JavaScript wird schon bezeichnet als Assembler für das Internet ("`assembly language for the web"') bzw. Virtual Machine des Internets ("`VM of the web"', Douglas Crockford)}
\TODO{  url %-->  \url{http://www.hanselman.com/blog/JavaScriptIsAssemblyLanguageForTheWebSematicMarkupIsDeadCleanVsMachinecodedHTML.aspx}
}
\TODO{  url %--> \url{http://www.hanselman.com/blog/JavaScriptIsAssemblyLanguageForTheWebPart2MadnessOrJustInsanity.aspx}
}
\TODO{  - JavaScript ist eine Plattform %\cite{doeraene2015.SSP}
}

\TODO{zwei Kategorien}
\TODO{1. ausgehend von JavaScript Verbesserung der Sprache durch angenehmere Syntax oder statische Typisierung}
\TODO{	- CoffeeScript (nicht getypt)}
\TODO{  - TypeScript (getypt, Obermenge von JavaScript, d.h. JS ist immer auch TS)}
\TODO{	Kritik von Doeraene: -> wenig Gewinn an Expressivität}
\TODO{2. andere statisch getypte Sprachen (extra neu entwickelte oder existierende), die nach JS kompilieren}
\TODO{	Dart, GWT (Java -> JS)}
\TODO{	Kritik von Doeraene: -> schlechte Interop mit JavaScript-libs, weil JS values nicht als first-class objs der host lang}
\TODO{cite %\cite[S. 1]{doeraene2013.TDI}
}

\TODO{Interop mit JavaScript-libs wichtig: DOM, andere Libs}
\TODO{cite %\cite[S. 1]{doeraene2013.TDI
}

\TODO{Code-Wiederverwendung}
\TODO{- traditionell verschiedene Sprachen für Client und Server}
\TODO{	- Client bei weitem dominiert von JavaScript (90 \% --> cite%\cite{w3techs.CLI}
)}
\TODO{	- Server (PHP, Java u.a. --> cite %\cite{w3techs.SRV}
)}
\TODO{- in dem Fall:}
...
\TODO{- Scala.js bringt Scala (bisher auf Basis der JVM) auf die Client-Seite:}
...
% % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % %
\chapter{Technikvorstellung/fachliches Umfeld/Grundbegriffe}
...
\section{Compiler und Optimizer // Builds/Entwicklungszyklus und minimierter/optimierter Produktionscode}\label{sec:compiler}
...
In der Optimierungsstufe (\code{FastOpt}) wird unter anderem unerreichbarer Code entfernt (\textit{Dead code elimination}) und es werden Methoden- und insbesondere anonyme Funktionsaufrufe ge-inlined (\textit{Closure elimination}), das heißt die Aufrufe werden durch ihre Implementierungen ersetzt. Hierdurch wird die Größe der Kompilate verringert und insbesondere die Performance verbessert. Das Ergebnis dieser Phase ist eine einzige JavaScript-Datei. Für die Entwicklungsphase endet der Prozess hier.
...
Machbarkeit
Lesbarkeit
Umsetzbarkeit aller Anforderungen
...
wichtige Anregungen aus Quelle
-->  Hands-on Scala.js
...
- Nachteile ...
	...
	- Doku momentan noch etwas (!) unübersichtlich und im Wandel
